...
`Age`: Patient Age, domain: `{python} f"[{describe.loc['min','Age']}, {describe.loc['max','Age']}]"`


`Deficit`: Base Deficit, measure of acidity, domain: `{python} f"[{describe.loc['min','Deficit']}, {describe.loc['max','Deficit']}]"`

`C-peptide`: Logarithm of C-Peptide concentration (pmol/ml), domain: `{python} f"[{describe.loc['min','C_peptide']}, {describe.loc['max','C_peptide']}]"`
...
The mean patient age is approximately `{python} round(describe.loc['mean','Age'])` years old, with the maximum age being approximately `{python} round(describe.loc['max','Age'])` and the minimum being `{python} round(describe.loc['min','Age'])`. The C_peptide concentration average is about `{python} f"{round(describe.loc['mean','C_peptide'], 2)}"`, mean Base Deficit value is `{python} f"{round(describe.loc['mean','Deficit'], 2)}"` mEq/L. The standard deviation of Base Deficit is quite high, at about `{python} f"{round(describe.loc['std','Deficit'], 2)}"` mEq/L, while the standard deviations of the other two variables are within a reasonable range for their domains. This high variance could be due to the small size of the dataset.
...
linear_reg = pd.read_csv("../results/tables/model_summary.csv", index_col=0)
linear_reg
```
...
shapiro = pd.read_csv("../results/tables/shapiro_wilk.csv", index_col=0)
shapiro
```
...
A multiple linear regression model (@tbl-linear_reg) yielded the following key findings:

Both predictor variables, Age (`{python} f"p = {float(linear_reg[linear_reg['names'] == 'Age']['pval'])}"`) and Deficit (`{python} f"p ={float(linear_reg[linear_reg['names'] == 'Deficit']['pval'])}"`), were found to be statistically significant in predicting the C-peptide level (using a typical $\alpha = 0.05$). Age has a positive coefficient (`{python} f"coef ={float(linear_reg[linear_reg['names'] == 'Age']['coef'])}"`), indicating that for every one-year increase in age, the log C-peptide concentration is expected to increase by `{python} f"{float(linear_reg[linear_reg['names'] == 'Age']['coef'])}"` units, holding Deficit constant. Base deficit has a positive coefficient (`{python} f"coef ={float(linear_reg[linear_reg['names'] == 'Deficit']['coef'])}"`), meaning that for every one $\text{mEq/L}$ increase in Deficit, the log C-peptide concentration is expected to increase by `{python} f"{float(linear_reg[linear_reg['names'] == 'Deficit']['coef'])}"` units, holding Age constant. (Note: Since Deficit values are negative, an increase (closer to 0) signifies less severe metabolic acidosis.)

The model achieved an adjusted $R^2$ of `{python} f"{linear_reg['adj_r2'][0]}"`. This means that approximately `{python} f"{linear_reg['adj_r2'][0]*100}%"` of the variance in the log C-peptide concentration is explained by the patient's age and base deficit.

When examining model assumptions, the diagnostic tests @tbl-sharpiro, @fig-qq, and @fig-residual, shows that the key linear regression assumptions of normally distributed residuals and equal variance are reasonably met. While it is more difficult to confirm whether the observations are independent, we will assume so here for the purposes of our analysis.
