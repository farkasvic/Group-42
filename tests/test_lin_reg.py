...
def check_dir(self, sample_data, temp_dirs):
...
def check_residuals(self, sample_data, temp_dirs):
    ...
    assert round(saved_data['residuals'], 2) == np.zeros(4)

def check_summary(self, sample_data, temp_dirs):
    ...
    # Check that the dataframe has expected columns from pingouin output
    assert round(results_df, 2) == pd.DataFrame({
...
def check_target_col(self, sample_data, temp_dirs):
...
def check_pickle_name(self, sample_data, temp_dirs):
...
def check_csv_name(self, sample_data, temp_dirs):
...
def check_model_dir(self, sample_data, temp_dirs):
...
def check_table_dir(self, sample_data, temp_dirs):
    ...
    with pytest.raises(FileNotFoundError, match="table_dir"):
        lin_reg(sample_data, 'target', model_dir, 
               "model.pickle", nonexistent_dir, "table.csv")

def check_pg_error(self, sample_data, temp_dirs):
